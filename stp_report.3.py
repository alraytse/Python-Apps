...
def check_hsrp(connection, vlan):
    """
    Check HSRP configuration and operational status on an SVI interface.
    Returns: ACTIVE, STANDBY, OTHER_STATE, CONFIGURED, or NONE
    """
    try:
        output = connection.send_command(
        ...
        if "state is active" in output_lower or " active " in output_lower:
            return "ACTIVE"
        if "state is standby" in output_lower or " standby " in output_lower:
            return "STANDBY"
        if any(state in output_lower for state in ["listen", "learn", "speak"]):
            return "OTHER_STATE"
...
def get_svi_info(connection, vlan, base_oid, oui_registry):
    """Extract SVI description, IP, MAC, company, HSRP status, and shutdown state."""
    description = ""
    ip_address = ""
    svi_mac = ""
    svi_company = ""
    ...
    hsrp_status = check_hsrp(connection, vlan)
