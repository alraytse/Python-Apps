...
def check_port_traffic(connection, target_list):
    ...
    for target in dict.fromkeys(targets):
        try:
            cmd = target if target.lower().startswith("vlan") else f"interface {target}"
            output = connection.send_command(
                f"show {cmd}",
                read_timeout=30,
            )
...
def check_hsrp(connection, vlan):
    ...
    try:
        ...
        if "invalid command" in output_lower or "hsrp not active" in output_lower or not output.strip():
            run_output = connection.send_command(
                f"show run interface vlan {vlan}",
                read_timeout=30,
            )
            return "hsrp " in run_output.lower()
        return "active" in output_lower or "standby" in output_lower or "hsrp" in output_lower
    except Exception:
        return False
    ...
    try:
        ...
        for vlan_info in vlans:
            ...
            if traffic_check == "TRAFFIC_DETECTED":
                print(f"Skipping VLAN {vlan_id} on {hostname}: Active traffic detected")
                continue
            ...
            svi_description, svi_ip, svi_mac, svi_company, hsrp_enabled = get_svi_info(
                connection,
                vlan_id,
                base_oid,
                oui_registry,
...
def display_results(results, max_mac_count, max_arp_count):
    ...
    for row in results:
        ...
        if row["SVI_MAC"]:
            print(f"{'':<8}{'SVI MAC: ' + row['SVI_MAC'] + ' | HSRP: ' + row['SVI_HSRP']}")
