...
def parse_vlans(output):
    """Parse VLAN IDs, names, and administrative shutdown/suspend states."""
    vlan_map = {}

    for line in output.splitlines():
        # Matches lines starting with VLAN ID, followed by name and status
        ...
        if match:
            ...
            is_shutdown = (
                "YES"
                if "shut" in status.lower() or "suspend" in status.lower()
                else "NO"
            )
...
def format_mac(mac, output_format="cisco"):
    ...
    if output_format == "colon":
        return ":".join(
            value[index : index + 2] for index in range(0, 12, 2)
        )
    ...
    return ".".join(
        value[index : index + 4] for index in range(0, 12, 4)
    )
...
def check_port_traffic(connection, target_list):
    """Check recent traffic rates on ports or VLAN interfaces."""
    targets = [
        target.strip()
    ...
    for target in dict.fromkeys(targets):
        try:
            cmd = (
                target
                if target.lower().startswith("vlan")
                else f"interface {target}"
            )
            output = connection.send_command(
                f"show {cmd}",
                read_timeout=30,
    ...
    return status, "; ".join(checked_targets)
...
def get_shutdown_recommendation(
    ...
    if vlan_shutdown == "YES" or svi_shutdown == "YES":
        down_target = []
        if vlan_shutdown == "YES":
            down_target.append("VLAN")
        if svi_shutdown == "YES":
            down_target.append("SVI")
        return (
            "ALREADY_SHUTDOWN",
            f"Interface or VLAN is already shut down ({' & '.join(down_target)})",
        )
...
def check_hsrp(connection, vlan):
    """Check if HSRP is configured or active on the SVI interface."""
    try:
        output = connection.send_command(
            f"show hsrp interface vlan {vlan}",
            read_timeout=30,
        )
        output_lower = output.lower()
        if (
            "invalid command" in output_lower
            or "hsrp not active" in output_lower
            or not output.strip()
        ):
            run_output = connection.send_command(
                f"show run interface vlan {vlan}",
                read_timeout=30,
            )
            return "hsrp " in run_output.lower()
        return (
            "active" in output_lower
            or "standby" in output_lower
            or "hsrp" in output_lower
        )
    except Exception:
        return False
...
def get_svi_info(connection, vlan, base_oid, oui_registry):
    """Extract SVI description, IP, MAC, company, HSRP, and shutdown status."""
    description = ""
    ip_address = ""
    svi_mac = ""
    svi_company = ""
...
def process_switch(
    ...
    try:
        ...
        for vlan_info in vlans:
            vlan_id = str(vlan_info["vlan"])

            # Exclude VLAN 1
            if vlan_id == "1":
                continue

            # Check traffic on the SVI/VLAN interface
            traffic_check, traffic_ports = check_port_traffic(
                connection,
                f"vlan{vlan_id}",
            )

            # Skip capture if active traffic was detected
            if traffic_check == "TRAFFIC_DETECTED":
                print(
                    f"Skipping VLAN {vlan_id} on {hostname}: Active traffic detected"
                )
                continue
            ...
            # Skip capture if MAC addresses or ARP entries exceed thresholds
            ...
            vlan_shutdown = vlan_info.get("vlan_shutdown", "NO")
            ...
            results.append(
                {
                    ...
                    "VLAN_Shutdown": vlan_shutdown,
                    "SVI_Shutdown": svi_shutdown,
                    "SVI_IP": svi_ip,
                    "SVI_MAC": svi_mac,
                    "SVI_MAC_Company": svi_company,
                    "SVI_HSRP": hsrp_enabled,
...
def display_results(results, max_mac_count, max_arp_count):
    ...
    for row in results:
        ...
        print(
            f"{row['VLAN']:<8}"
            f"{row['VLAN_Name']:<20}"
            f"{row['VLAN_Shutdown']:<11}"
            f"{row['SVI_Shutdown']:<10}"
            f"{row['SVI_IP']:<18}"
            f"{row['SVI_MAC']:<18}"
            f"{row['MAC_Count']:<6}"
            f"{row['ARP_Count']:<6}"
            ...
            f"{row['ARP_Check']:<12}"
            f"{row['Root_Bridge']:<6}"
            f"{row['Shutdown_Recommendation']:<18}"
        )
        if row["SVI_MAC"]:
            print(
                f"{'':<8}{'SVI MAC: ' + row['SVI_MAC'] + ' | HSRP: ' + row['SVI_HSRP']}"
            )
    ...
    root_count = sum(1 for row in results if row["Root_Bridge"] == "YES")

    recommendation_counts = {
        level: sum(
            1 for row in results if row["Shutdown_Recommendation"] == level
        )
        for level in ("STRONG", "MODERATE", "WEAK", "ALREADY_SHUTDOWN")
    }
    ...
    print(f"Already Shutdown        : {recommendation_counts['ALREADY_SHUTDOWN']}")
    print("=" * 270)
...
def main():
    ...
    hosts = input("Enter switch hostnames/IPs (comma delimited): ").strip()
